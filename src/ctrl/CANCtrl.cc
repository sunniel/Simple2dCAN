...
void CANCtrl::r_transmit(CANMessage* msg, unsigned long destID) {
    if (GlobalNodeListAccess().get()->isUp(destID)) {
        ...
        IPvXAddress srcAddr = GlobalNodeListAccess().get()->getCANAddr(
                info.getId());
        IPvXAddress destAddr = GlobalNodeListAccess().get()->getCANAddr(destID);
        ...
        if (label.find("init") != string::npos) { // for overlay initialization
            sendIdeal(msg);
        } else {
        ...
        } else if (msg->getType() == CANMsgType::LOOK_UP
                || msg->getType() == CANMsgType::STORE
                || msg->getType() == CANMsgType::GET) {
...
void CANCtrl::onRoute(CANMessage* msg) {
    ...
    if (!includedInArea(p, nextHopArea)
            ...
            if (next.isEmpty()) {
                ...
                if (msg->getType() == CANMsgType::JOIN) {
                    ...
                    IPvXAddress senderAddr =
                            GlobalNodeListAccess().get()->getCANAddr(
                                    msg->getSender());
        ...
        if (msg->getType() == CANMsgType::LOOK_UP) {
            CANMessage* finalmsg = new CANMessage(msg::CAN_FINAL);
            finalmsg->setType(CANMsgType::FINAL);
...
//            finalmsg->setContent(profile.dump().c_str());
        ...
        } else if (msg->getType() == CANMsgType::STORE) {
            ...
            cout << simTime() << " Store value on " << info << endl;
        } else if (msg->getType() == CANMsgType::GET) {
            GlobalStatisticsAccess().get()->addHop(msg->getHop());
            if (data.count(p) > 0) {
                ...
                cout << "Return GET result: " << data::CAN_DATA_EMPTY
                        << " from " << info << endl;
                GlobalStatisticsAccess().get()->FAILS++;
            }
            ...
            if (!takeovers.empty()) {
                ...
                if (subzones.size() > 0) {
                    ...
                    // TODO update Virtual ID

                    // TODO update zone overload
...
void CANCtrl::onJoinReplyFail(CANMessage* msg) {
    ...
    if (label.find("init") != string::npos) {
        UnderlayConfiguratorAccess().get()->initNodeAdded();
    }
...
void CANCtrl::onJoinReply(CANMessage* msg) {
    ...
    GlobalNodeListAccess().get()->updateCANProfile(info);
    GlobalNodeListAccess().get()->addZone(info);
    GlobalNodeListAccess().get()->canReady(info.getId());
    startMaint();
    ...
    for (auto elem1 : neighborsInCharge) {
        ...
        if (entries.count("data") > 0) {
            for (int i = 0; i < neighbors.size(); i++) {
                ...
                replicate->setSender(info.getId());
                replicate->setType(CANMsgType::FIX_REPLICA);
