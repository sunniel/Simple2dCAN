...
public:
    ...
    bool isUp(unsigned long canId);
    bool isReady(unsigned long canId);
    void canReady(unsigned long canId);

    IPvXAddress getCANAddr(unsigned long canId);
...
protected:
    ...
    // the following set does not contains a customized comparator for WATCH, which may cause view inconsistency
    set<CANProfile> zoneInCharges;
    /*
     * record the online state of CAN node, which will be ready for service
